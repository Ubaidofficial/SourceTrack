...
**Remaining (B3 step 4):** delete `queryPostHog` (its only remaining callers are the reads-DISABLED conversions + touchpoints legs), the `:175` boot guard, the dead `validateSite` skip guard, and two stale comments — after which D5 is unblocked. Not done here.
