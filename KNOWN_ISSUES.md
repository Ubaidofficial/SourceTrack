...
### 🔴 PostHog legal copy is gated on confirming the PostHog project is actually DELETED (2026-07-19, D5)
PostHog is fully off the code path (no read/write, dependency + frontend client removed). But the **historical customer data may still RESIDE in the PostHog project** — the repo has **no PostHog data-deletion job or runbook**, and `COMMANDCODE_RUNBOOK.md:441/453` explicitly states historical events are **not** bulk-deleted and retention "must be verified in the provider console." Until the founder confirms the PostHog project/data is deleted:
