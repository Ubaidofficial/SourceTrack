...
### Dead CAPI senders: LinkedIn FIXED (#514) — Microsoft remains, and needs an OAuth2 rewrite, not a config entry (filed 2026-07-23 · updated 2026-07-30 @ `5c03200`)

> **STATUS 2026-07-30: half closed.** **LinkedIn shipped in #514 and is no longer dead.** **Microsoft is the one real remaining gap** — and the reason is sharper than this entry originally recorded, so the diagnosis below is corrected rather than just re-dated. **Deliberately backlogged, not forgotten** — revisit on real customer demand, per the Reddit/Snapchat precedent, not proactively.

**LinkedIn — CLOSED (#514, `1754559`).** `linkedin` is now in `CAPI_PLATFORMS` (`api/routes/capi.js:52`) with `tokenCol: 'linkedin_capi_token'` / `idCols: { partner_id: 'linkedin_partner_id' }`. No migration was needed — both columns already existed on prod via the baseline schema plus `20260712000000_backfill_sites_capi_google_ads_columns.sql`, and both were already in the `SELECT` lists. `sendLinkedInConversion` already Bearer-authenticates against `api.linkedin.com/v2/conversionEvents` and was already in the fan-out, so the config card was genuinely the last missing touchpoint. **`CAPI_PLATFORMS` is now five platforms: `meta`, `google`, `ga4`, `tiktok`, `linkedin`** (GA4 + TikTok arrived in #498).

**Microsoft — STILL OPEN, and the original diagnosis here was incomplete.** Verified at `5c03200`:

- The "missing columns" half is **no longer true**: `microsoft_tag_id` and `microsoft_capi_token` **are** in both hardcoded `SELECT` lists (`api/routes/conversion.js:423`, `api/routes/conversion-offline.js:232`), and `sendMicrosoftConversion` **is** registered in the fan-out (`api/lib/conversion-sync.js:473`).
- The **real** blocker: `sendMicrosoftConversion` (`conversion-sync.js:232`) POSTs to `https://bat.bing.com/bat.svc/c` — the UET **tracking** endpoint — with `Content-Type` as its **only** header. It reads `microsoft_capi_token` at `:236` **solely to check the token is present and decryptable** (`:237` returns `null` if not) and then **never transmits it**. Its own comment defers that to "Phase 2".
- So adding a Microsoft card to `CAPI_PLATFORMS` would **save a credential that is never sent** — a config card that reads as connected while the token does nothing. That is precisely the failure the (now-enforced) membership rule above `CAPI_PLATFORMS` forbids, which is why Microsoft is held **on purpose** rather than overlooked.
- **Finishing it is a sender rewrite plus new columns, not a config-map entry.** Microsoft offline conversions run through the Microsoft Advertising **Campaign Management API**, which needs **OAuth2: developer token + customer ID + account ID + refresh/access token** — a different credential *shape* from the `tag_id + token` pair the current column pair models. Both the rewrite and the migration are unscheduled.

**Root cause worth recording — adding a CAPI platform needs FOUR touchpoints in lockstep:** (a) the sender fn in `conversion-sync.js`, (b) an entry in `CAPI_PLATFORMS` (`capi.js`), (c) a config card in `dashboard/src/components/CapiSettings.jsx`, (d) the hardcoded CAPI-column `SELECT` lists in `conversion.js` + `conversion-offline.js`. Miss any one and the sender is stillborn — exactly how Microsoft/LinkedIn ended up here. This is the checklist for any future platform. **Validated in practice since:** GA4 + TikTok (#498) and LinkedIn (#514) all shipped by walking exactly these four, and the rule is now written into the comment above `CAPI_PLATFORMS` — a platform is admitted **only** once column, config card **and** live forwarding all exist. Microsoft fails a *fifth* condition this checklist never anticipated: its sender reaches all four touchpoints yet still cannot work, because the endpoint it posts to does not accept the credential at all (see the status block above).

**Not a blocker, and the delivery log needs no change:** `capi_deliveries` is platform-agnostic (`api/lib/capi-deliveries.js` — columns `site_id, platform, event_ref, status, http_status, error_message, attempt`; `platform` is a bare string), so it already records any platform without a schema change. **Decided 2026-07-30:** LinkedIn was wired through all four touchpoints (#514); Microsoft is **kept but held** — not dropped, because the sender is a real head start on the OAuth2 rewrite, and not exposed, because a card would save a token that is never sent.
