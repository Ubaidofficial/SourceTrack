...
**Remaining (B3 step 3/4):** the **touchpoints** read inside `processConversion` still falls to `queryPostHog` on null (a null → attributes with zero touchpoints, silently), and **`queryPostHog` itself** (now two callers: reads-disabled conversions + touchpoints) is not yet deleted — after which the `:175` boot guard can go. Not done here.

**No wall-clock ceiling exists.** The nightly has a 6h concurrency LOCK (`LOCK_TTL_HOURS`, `nightly-attribution.js:189`) but that is a **start-time guard** (a new run refuses to start if the prior run is still `status='running'` and started <6h ago) — nothing **kills** a long run. There is no `railway.json`/`railway.toml` in the repo defining a cron timeout. So a fully-degraded run has no internal deadline; if it exceeds 6h the lock expires and the next cron could overlap it. Step 2's per-site short-circuit bounds a fully-degraded **conversions** read to ~(sites × 51s) — but a site that serves conversions yet has degraded **touchpoints** (step 3 scope) can still retry ~51s per conversion, which is the remaining unbounded path.
