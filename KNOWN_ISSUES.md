...
1. **`total_conversions` is page-scoped** (`leads-server.js:220`). `total_conversions = leads.reduce((s,l)=>s+l.conversions,0)` sums over the **returned page only**, so the KPI changes with pagination and undercounts past the limit. Proof: a search that matches nothing drops it to 0 (it's a sum over returned rows); the observed "2" was exactly the two 7/14 form conversions on the visible page, with the two 7/09 converters beyond row 100. **Fix:** replace the `reduce()` with a **full-window server-side count** — same shape as the `leads_count` pipe (a windowed `countIf($conversion)` / `sum(conversions)` aggregate), **NOT** a bigger page.

2. **KPI/table label mismatch.** "TOTAL LEADS" (tile) and "N leads in range" (table subtitle) both use `total` = `leads_count` = distinct **converters**, but they sit over the `leads_list` table (`HAVING conversions>0 OR pageviews>0` = converted-OR-engaged, 100+ rows). **Fix:** give the table its OWN count (the true `leads_list` population size), and keep distinct-converters as a **separately-labeled** KPI — don't use one number to label the other.
