  ...
  return (
    <div className="w-full">
      ...
      <div className="text-center mb-8">
        ...
        <p className="mt-2 text-[#A8AFAF] text-sm sm:text-base max-w-[620px] mx-auto font-medium">
          Explore sample analytics, source attribution, AI traffic, and conversion journeys — no account needed.
        </p>
      </div>

      {/* Browser frame container */}
      <div className="relative rounded-[24px] sm:rounded-[36px] bg-[#1F2323] p-[10px] sm:p-[14px] shadow-[0_34px_110px_rgba(27, 31, 31,.34)] border border-white/20">
        <div className="overflow-hidden rounded-[18px] sm:rounded-[26px] bg-[#1F2323] border border-[#303636]">
          {/* Top Browser Bar */}
          <div className="h-[52px] sm:h-[60px] grid grid-cols-[auto_1fr_auto] items-center gap-2 sm:gap-4 px-3 sm:px-[18px] bg-[#1B1F1F] border-b border-white/10 text-[12px] font-bold text-[#F2F4F3]">
            {/* Mac style dots */}
            <div className="flex gap-[6px] sm:gap-[7px]">
              <span className="w-[8px] h-[8px] sm:w-[10px] sm:h-[10px] rounded-full bg-[#E54545]" />
              <span className="w-[8px] h-[8px] sm:w-[10px] sm:h-[10px] rounded-full bg-[#F0602A]" />
              <span className="w-[8px] h-[8px] sm:w-[10px] sm:h-[10px] rounded-full bg-[#CCF03F]" />
            </div>

            {/* Site Pill Search Bar */}
            <div className="mx-auto w-[160px] sm:w-[280px] bg-[#1F2323] border border-white/10 rounded-lg py-1 px-2.5 sm:px-4 text-[11px] font-mono text-[#7D8090] text-center truncate">
              demo.sourcetrack.ai
            </div>
          ...
          <div className="p-3 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-[#1F2323] border-b border-white/5">
            {/* Mode Switcher */}
            <nav className="flex items-center bg-[#282C2C] border border-white/10 rounded-full p-0.5 self-start" aria-label="Demo scenario selector">
              {[
                ['SaaS', 'SaaS'],
                ['eCommerce', 'eCommerce'],
                ['LeadGen', 'Lead Gen'],
                ...
                <button
                  ...
                  className={`px-3 sm:px-4 py-1.5 rounded-full text-xs font-extrabold transition-all focus:outline-none focus:ring-2 focus:ring-[#CCF03F] ${
                    activeMode === key
                      ? 'bg-[#CCF03F] text-[#1F2323] shadow-md'
                      : 'text-[#A8AFAF] hover:text-white'
                  }`}
                >
                  {label}
                </button>
            ...
            <div className="flex items-center gap-2 text-xs font-bold text-[#A8AFAF]">
              <span className="px-3 py-1.5 bg-[#282C2C] border border-white/10 rounded-lg text-white">
                Last 30 days
              </span>
            </div>
          </div>
          ...
          <div className="p-3 sm:p-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {modeData.metrics.map((m) => (
              <div
                key={m.id}
                className="bg-[#1B1F1F] border border-[#303636] rounded-xl p-3 sm:p-4 min-h-[92px] sm:min-h-[108px] flex flex-col justify-between"
              >
                ...
                <div className="mt-1 text-[10px] sm:text-xs font-bold text-[#CCF03F]">
                  {m.trend}
                </div>
              </div>
            ))}
          ...
          <div className="px-3 sm:px-5 pb-3 sm:pb-5">
            <div className="bg-[#1B1F1F] border border-[#303636] rounded-xl p-4 sm:p-5">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <h4 className="text-[#A8AFAF] text-[11px] font-black uppercase tracking-[0.05em]">
                ...
                <div className="text-right text-xs bg-[#282C2C] border border-white/5 px-3 py-1.5 rounded-lg text-white font-mono">
              ...
              {/* Chart Grid */}
              <div className="relative h-[120px] sm:h-[160px] flex items-end gap-2.5 sm:gap-4 pt-6 border-b border-[#303636]">
          ...
          <div className="px-3 sm:px-5 pb-5 grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr_1fr] gap-4">
            ...
            <div className="bg-[#1B1F1F] border border-[#303636] rounded-xl p-4 sm:p-5 flex flex-col justify-between min-h-[300px]">
              <div>
                {/* Tab selector A */}
                <div className="flex border-b border-[#303636] pb-2 mb-3" role="tablist">
                  {[
                    ['sources', 'Sources'],
                    ['ai', 'AI Sources'],
                    ['pages', 'Top Pages']
                    ...
                    <button
                      ...
                      className={`mr-4 pb-2 text-xs font-black transition-all border-b-2 -mb-[10px] focus:outline-none focus:text-white ${
                        activeTabA === key
                          ...
                          : 'border-transparent text-[#7D8090] hover:text-[#A8AFAF]'
                      }`}
                    >
                      {label}
                    </button>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs font-semibold text-white">
                    <thead>
                      <tr className="text-[#7D8090] border-b border-[#303636] text-[10px] font-black uppercase tracking-wider">
                    ...
                    <tbody>
                      {modeData.tables[activeTabA].map((row) => {
                        ...
                        return (
                          <tr
                            key={row.name}
                            className={`group border-b border-[#303636] last:border-0 hover:bg-white/5 transition-colors ${
                          ...
                          >
                            <td className="py-2 pr-2 font-bold truncate max-w-[120px]">
                              <button
                                ...
                                className={`w-full text-left font-bold focus:outline-none focus:ring-2 focus:ring-[#CCF03F] rounded px-1 -mx-1 py-1 flex items-center gap-1.5 transition-all ${
                            ...
                            <td className="py-2 text-right font-mono pr-2 text-[#F2F4F3] group-hover:text-white">
                              {row.visitors}
                            </td>
                            <td className="py-2 text-right font-mono pr-2 text-[#F2F4F3] group-hover:text-white">
                  ...
                  <table className="w-full text-left text-xs font-semibold text-white">
                    ...
                    <tbody>
                      {modeData.tables[activeTabB].map((row) => (
                        <tr
                          key={row.name}
                          className="border-b border-[#303636] last:border-0 hover:bg-white/[0.02] transition-colors"
                        >
                          <td className="py-2.5 pr-2 font-bold truncate max-w-[120px] text-white">
                            {row.name}
                          </td>
                          <td className="py-2.5 text-right font-mono pr-2 text-[#F2F4F3]">
                            {row.visitors}
                          </td>
                          <td className="py-2.5 text-right font-extrabold text-[#F2F4F3]">
                            {row.revenue}
                          </td>
                        </tr>
                      ))}
            ...
            <div className="bg-[#1B1F1F] border border-[#303636] rounded-xl p-4 sm:p-5 flex flex-col justify-between min-h-[300px]">
              <div>
                <h4 className="text-white text-xs font-black border-b border-[#303636] pb-2 mb-3">
                ...
                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mt-2">
                  {journey.steps.map((step, idx) => (
                    <React.Fragment key={idx}>
                      ...
                      <span className="px-2.5 py-1 rounded-full bg-[#282C2C] text-[#F2F4F3] text-[10px] font-bold border border-white/10 truncate max-w-[120px]" title={step}>
                        {step}
                      </span>
                    </React.Fragment>
                  ))}
                ...
                {journey.explanation && (
                  <p className="text-[#A8AFAF] text-[11px] leading-relaxed mt-3 bg-[#282C2C] p-2.5 rounded-lg border border-white/5 font-semibold">
                    {journey.explanation}
                  </p>
                )}
                ...
                {journey.recommendedTemplate && (
                  <div className="mt-3 p-2.5 rounded-lg bg-[#CCF03F]/10 border border-[#CCF03F]/20 text-[10px] font-black text-[#CCF03F]">
                ...
                <div className="grid grid-cols-2 gap-2 mt-4 text-[10px] font-bold">
                  <div className="p-2 rounded-lg bg-[#282C2C] border border-white/5 truncate">
                  ...
                  <div className="p-2 rounded-lg bg-[#282C2C] border border-white/5 truncate">
                    ...
                    <div className="mt-0.5 text-[#CCF03F] truncate" title={journey.status}>{journey.status}</div>
                  </div>
                </div>

                <div className="mt-3 p-2 rounded-lg bg-[#282C2C] border border-white/5 text-[10px] font-bold">
              ...
              <div className="mt-4 pt-3 border-t border-[#303636] text-[10px] text-[#7D8090] flex items-center justify-between">
                <span>Stitched visitor journey</span>
                <span className="px-1.5 py-0.5 bg-[#303636] text-[#CCF03F] text-[9px] rounded font-black font-mono">STITCHED</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-8 text-center flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
        <a
          ...
          className="w-full sm:w-auto inline-flex items-center justify-center min-h-[50px] px-8 rounded-full bg-[#CCF03F] text-[#1F2323] text-[15px] font-extrabold tracking-[-0.02em] shadow-[0_12px_44px_rgba(204, 240, 63,0.18)] hover:bg-[#D9FA64] transition-all hover:-translate-y-px"
        >
          Start tracking free
        </a>
        <a
          href="/pricing"
          className="w-full sm:w-auto inline-flex items-center justify-center min-h-[50px] px-8 rounded-full border border-white/10 bg-[#1F2323] text-white text-[15px] font-extrabold tracking-[-0.02em] hover:border-white/25 transition-all hover:-translate-y-px"
        >
          See pricing
        </a>
      </div>
