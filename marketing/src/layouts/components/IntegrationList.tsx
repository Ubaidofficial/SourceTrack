...
const IntegrationList = ({
  ...
  const filteredList = items.filter((integration) => {
    const titleStr = integration.frontmatter?.title || integration.frontmatter?.page_header?.title || "";
    const matchesSearch = titleStr
      .toLowerCase()
                ...
                <div className="group relative flex cursor-pointer flex-col items-center justify-center overflow-hidden rounded-3xl bg-primary-dark py-12 px-8 text-center border border-border/6 h-full">
                  ...
                  <div className="relative z-10 flex flex-col items-center justify-center">
                    <div className="mb-7 flex h-10 w-10 items-center justify-center">
                      <ImageFallback
                        src={integration.frontmatter.icon || integration.frontmatter.image || "/images/integrations/stripe.svg"}
                        alt={integration.frontmatter.title || "Integration"}
                        width={40}
                        height={40}
                    ...
                    <h3 className="mb-3 text-2xl font-bold text-text">
                      {integration.frontmatter.title || integration.frontmatter.page_header?.title}
                    </h3>

                    <p className="text-gray">
                      {integration.frontmatter.description || integration.frontmatter.page_header?.subtitle}
                    </p>
                  </div>
                </div>
              </a>
