...
| 14 | `sessions` + `conversion_rate` (🚧 @ `cb17cc2`) | Gated ENTIRELY — dead on every dim; never routed to the session pipes | Unique-Visitors-by-dim + the `univ_cvr` template are non-functional until a pipe lands (§3). ✅ **Gate re-confirmed @ `c9a4113`** (`report-config-validation.js:331` denies on `GATED_METRICS`). **DOWNGRADED urgent→backlog this session:** the only live exposure was `api/routes/campaigns.js`, closed by **#409**; `attribution.js`/`export.js` both call `gatedReportReason()` before `getFlexibleReport()`, and the internal `multiTouchAttributionHelper` hardcodes `metric:'revenue'`. |
