...
| 8 | Stripe/Shopify buyer-attribution | Webhooks built; refund netting added (Stripe #381, Shopify #384) | No prod site connected → not live; no real refund processed; Tinybird refund-count fix (#383) authored but undeployed |
...
| 15 | Campaigns page | ⚠️ 1 of 4 tabs (UTC) / 0 of 4 (non-UTC) | The 3 campaign pipes **are deployed and do serve** (the old "INERT/undeployed" note was wrong). `campaign` works on UTC; `source`/`medium`/`ai_source` 422 for every site (**KI-53**), and non-UTC loses `campaign` too (**KI-51**). Honest "temporarily unavailable" render, not fake zeros (§8) |
