...
- ⚠️ **health-agent: detection correct, delivery absent.** Scheduling is **confirmed** (Railway `f15924b7`, `*/30 * * * *`, `node api/jobs/health-agent.js`) — not "unconfirmed". It does **NOT** monitor PostHog: that check was **RETIRED in D2**, `data_flow` reads Tinybird, and `CRITICAL_CHECKS = {supabase, nightly_job, conversions, tinybird_quarantine}` (`health-agent.js:18`). The real gap is subtler than "unset": **`SLACK_WEBHOOK_URL` in prod holds a truthy placeholder**, so `notify()` does **NOT** return early at `health-agent.js:283` (`if (!SLACK || dx.severity === 'ok') return`). It instead POSTs every critical alert to a **dead Slack path** at `:289`, where the `fetch` has **no `.ok` check and no `try/catch`** — the 404 is swallowed and the run reports nothing wrong. Had the var been genuinely unset, `:283` would discard the alert silently instead; **both paths lose every critical result.** `notify()` is also **unwrapped**: a network *throw* (not a 404) rejects `run()`, the top-level `.catch` (`:328`) logs a generic crash, and `process.exit(snap.overall === 'critical' ? 1 : 0)` (`:322`) never runs — masking the verdict. health-agent additionally writes **no `job_runs` row** (its own runs are unobservable). `KNOWN_ISSUES` 29.
- ✅ **Quarantine alarm** (`quarantine-alarm.js`, silent-revenue-loss monitor) — built + tested + **wired into health-agent (#97)**: imported at `health-agent.js:5`, `tinybird_quarantine` is in `CRITICAL_CHECKS` (`:18`). ⚠️ Caveat: like every critical check, its alert is **lost** — `SLACK_WEBHOOK_URL` holds a truthy placeholder, so `notify()` POSTs to a dead Slack path whose 404 is swallowed by the unchecked `fetch` (see the health-agent row above).
...
| 7 | health-agent + quarantine alarm | ✅ built + wired (#97), scheduled `*/30` in prod, on Tinybird | **Delivery gap:** `SLACK_WEBHOOK_URL` holds a truthy placeholder → `notify()` POSTs alerts to a dead Slack path, 404 swallowed (unchecked `fetch`, no `try/catch`, `:289`); a genuinely unset var would drop them at `:283` instead — both lose every critical result. `notify()` is unwrapped, so a network throw masks the exit verdict (`:322`/`:328`). Writes no `job_runs` row (`KNOWN_ISSUES` 29) |
