...
- **Merged-identity (`visitor_id != distinct_id`)** — every Phase 4/5 fixture is single-identity (`distinct_id == visitor_id`). Identity merge has **two layers**: `server-events.js:69-85` (the stitching decision — anonymous_id vs user_id → `distinct_id`, incl. `resolveAnonymousId`) and `tinybird/adapter/normalize.js:251-253` (the column mapping onto the Tinybird row — `visitor_id = src.visitor_id ?? src.anonymous_id ?? distinctId`, where divergence materializes). No fixture exercises it. **Harness marker: HARD-SKIP with "MERGED-IDENTITY UNCOVERED" banner** across `last_touch` + `ai_platforms` (and any join/IN-list-keyed model). Requires a seeded merged-identity fixture (§9 list; construction is a founder decision — see §11 OPEN).

> **Anti-false-green rule:** the harness reports `PASS (6/9 row-level signed-off · 3/9 recorded-pending-ingest · 0/9 aggregate · 2 paths hard-skipped)` — it must NEVER print "all-9 green" while any recorded-pending-ingest / aggregate-pending / HARD-SKIP marker is active. (Models 7/8/9 are RECORDED to disk but NOT yet ingested/run — they count as "green" only after founder ingest + a passing row-level diff.)
...
| **Merged-identity fixture** (`visitor_id != distinct_id` via `user_id`-merge) | un-gate the 2 HARD-SKIP paths (`last_touch`, `ai_platforms` merged-identity) | ❌ MUST SEED — construction is a founder decision (§11 OPEN #6); `SESSION_HANDOFF.md:25, :40, :41` |
...
- **#6 merged-identity fixture — construction method.** OPEN: founder decision — **Branch A** (`user_id`, events-only, ~half-day, no governance) vs **Branch B** (`user_id_resolved`, needs a `site_identity_links` Supabase seed). Events-only requires a correctness cross-check vs `server-events.js:69-85` + `tinybird/adapter/normalize.js:251-253` or it false-passes. Requires a generator extension that breaks the `distinct_id == visitor_id` invariant.
