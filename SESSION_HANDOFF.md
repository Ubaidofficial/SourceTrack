...
> - **HEAD:** `d06ead7` on `claude/tinybird-phase1-events-schema` (pushed). Builds on `TINYBIRD-PHASE4-COMPLETE` below — this entry covers Phase 5 (mechanical endpoint ports) through the first live Tinybird deployment and a partial Phase 6 (N+1 rewrite + consolidation audit).
> - **Phase 5 SIGNED OFF — 54 endpoint pipes ported + parity-verified** (value-level diffs vs live HogQL on PostHog 469905, gating site `de200000-...441111`, not count-only): Bucket A (39, non-identity: setup-doctor/alerts/integrations/events/admin/dash) + B1 (10, `distinct_id`-keyed) + B2 (4, `argMin`/`argMax` reads: `leads_list`, `lead_detail`, `cohorts_ai`, `seo_revenue_landing_pages`) + B3 (6, bag `anonymous_id` cluster) + 5 hygiene pipes closed-by-reuse. Plus 2 Phase-4 pipes fixed (`last_touch_by_site`, `pageviews_by_visitors`: `visitor_id`→`distinct_id`, #19c). **60 total `.pipe` files** at deploy time (54 shared-token + 5 Phase-4 per-pipe-token + 1 materialized `events_by_visitor_mv`).
> - **Identity-key correction (B3):** the dispatch's premise that `analytics.js`'s 5 reads (`summary`/`sources_ai`/`sources_ref`/`browsers`/`os`) dedup on bag `properties.anonymous_id` was **wrong** — traced `fetchPageviews`'s `PAGEVIEW_FIELDS` map (`posthog.js:75-88`) and found it aliases output column `anonymous_id` to the raw `distinct_id` typed column (code's own comment: "visitor-dedup uses anonymous_id (distinct_id)"). Porting as bag would have broken parity. **Only `live.js:22` genuinely reads bag `properties.anonymous_id`** — confirmed verbatim. Ported each faithfully to what its own source actually does, not the blanket assumption. Caught a real bag-field bug in the same batch: `JSONExtractString` returns `''` (never NULL) for a missing key, so naive `count(DISTINCT ...)` over-counts by the empty-string bucket — wrapped `nullIf(...)` to match HogQL's NULL-exclusion (11=11 exact after fix, vs 12 naive).
> - **`argMin`-over-first_touch NULL-vs-DEFAULT finding (B2, cross-cutting, affects the whole class not just B2):** the batch/normalize adapter maps `null`→column DEFAULT at ingest (`first_touch_campaign`→`''`, `first_touch_source`→`'direct'`, `first_touch_medium`→`'none'`, `conversion_type`→`'untyped'`), destroying the "genuinely-NULL vs explicitly-set-to-default" distinction before Tinybird sees it. Live HogQL's `argMin` skips NULL but keeps `''`; neither `nullIf(x,DEFAULT)` nor no-`nullIf` reproduces this faithfully — proved both directions wrong on real cross-store fixtures. **Decision: do NOT refactor schema mid-migration** (bounded divergence — zero affected visitors on gating site; app read-layer already coalesces NULL→'direct'). Left `campaign`/`ft_source`/`ft_medium` unfixed (correct for the converting-lead population); deferred the Nullable-vs-DEFAULT DDL decision to a dedicated pre-cutover review (Phase 7/8). Separate clean fix kept: `argMaxIf(conversion_type,...)` returns `''` not NULL on zero-match → wrapped `nullIf(...,'')`.
> - **SEO-revenue truth-gate fixed (app layer, `01e665f`):** `SEORevenue.jsx` rendered `$0` instead of hiding when no matched SEO revenue existed (violated §5.1/§5.2 no-fake-zeros). Fixed to gate on `revenue > 0` (founder's explicit choice — hides `$0` even for a genuine zero-value conversion, for a cleaner revenue view; tradeoff: a real $0 conversion is now visually indistinguishable from "no data," known and accepted). §5.4 estimate label added adjacent in 3 spots (KPI tile, table, query panel). **SEO-revenue migration parity is separately NOT proven** — `seo_revenue_landing_pages`'s organic-search filter path has zero fixture coverage (every `utm_source=google` fixture on the gating site is paid/cpc); founder must seed a dedicated organic fixture pre-cutover.
> - **FIRST LIVE TINYBIRD DEPLOY — Deployment #4, live in ST_Staging** (`imubaid93_workspace`, gcp/europe-west3, Enterprise), auto-promoted. 56 pipes deployed. First non-read-only Tinybird action of the whole migration — every prior phase was `.pipe` authoring + read-only `tb --cloud sql` only. App behavior unchanged (`TINYBIRD_READ_ENABLED` unset).
> - **Read-token model, corrected from the original plan:** the intended "one shared `PIPES:READ` static token" is **not buildable on `tb` v4.6.4** — static tokens support only `ADMIN`/`TOKENS`/`WORKSPACE:READ_ALL`/`WORKSPACE:DEPLOY`/`ORG_DATASOURCES:READ`; `PIPES:READ` is JWT-only (has a TTL, per-`--resource`). Created `st_endpoint_read` as a static `WORKSPACE:READ_ALL` token instead — the honest adaptation, broader than intended (reads the entire workspace, not just the 49 endpoint pipes), accepted for staging. **Prod (Phase 10) should reconsider per-tenant JWTs.** The 54 pipes' pipe-level `TOKEN "st_endpoint_read" READ` line was **stripped** (it conflicted with the workspace-scoped token — `tb`'s own required fix, confirmed via `deploy --check`). The 5 Phase-4 pipes kept per-pipe tokens (auto-created on deploy: `phase4_conversions_read`, `phase4_last_touch_read`, `phase4_pageviews_by_visitors_read`, `phase4_pageviews_read`).
> - **`deploy --check` is a MANDATORY pre-deploy gate, not optional — new standing lesson.** Read-only `tb --cloud sql` with real values structurally cannot catch param-declaration or SQL-structure errors (it never sees the `'__no_value__'` placeholder path `--check` exercises). Caught ~15 deploy-blockers across 3 rounds that all of Phase 4/5's parity validation missed: **(1)** 11 pipes declared date params as `String` + `toDateTime()` wrapper → unparseable placeholder; fixed to `{{ DateTime(param, required=True) }}` direct (no wrapper), or `{{ DateTime(param, '1970-01-01 00:00:00') }}` for optional `{% if defined %}` params — matching the pattern the Phase-4 pipes already used correctly. **(2)** 3 `toTimeZone` pipes (`dashboard_bounce_rate`, `dash_top_pages`, `dash_stages`) declared `tz` as `String` with no default → "Cannot load time zone `__no_value__`"; fixed to `{{ String(tz, 'UTC') }}` (UTC = gating site's real configured tz too, confirmed via Supabase). **(3)** `cohorts_ai` `ILLEGAL_AGGREGATION`: inner `argMin(ai_source, timestamp) AS ai_source` collided with the raw column name inside the same aggregate — renamed inner alias to `ai_src`, re-aliased to `ai_source` in the outer `SELECT`. All 3 fix classes re-validated against live gating-site data post-fix; parity held exactly (W26 cohorts: ChatGPT=4, Perplexity=3, unchanged).
> - **Read-rail wire format VERIFIED end-to-end** via direct curl against the deployed endpoints (not `tb sql`) — the single biggest previously-unverified item. Confirmed: endpoint answers HTTP JSON; `st_endpoint_read` (`WORKSPACE:READ_ALL`) authenticates endpoint calls; single-param binding works (`doctor_pageviews_30d?site_id=...` → 32, matches parity); **array-param wire format works** — `pageviews_by_visitors` with `visitor_ids` passed as **repeated query keys** (`visitor_ids=X&visitor_ids=Y&visitor_ids=Z`) bound correctly and returned `cc-4a-j1-visitor`'s exact 3-touch Phase-4a fixture journey (google/cpc/brand → direct → newsletter/email, chronological) — confirms `tinybird-read.js`'s documented repeated-key convention is the correct wire shape, deployed-pipe==validated-pipe parity on the array path too. Sorting-key prune confirmed on both paths (`rows_read: 45` for a targeted query vs full-table). **Param contract for `pageviews_by_visitors`** (learned from the pipe's own template error, corrects an earlier guess): `site_id`, `visitor_ids` (array), `lookback_from` (not `date_from`), `date_to`.
> - **Phase 6 (N+1 rewrites + consolidation) — PARTIAL:**
>   - **`aiplatform:505` N+1 rewrite AUTHORED, safe-off, app-path NOT yet verified.** First read-side infra in the codebase (`api/lib/tinybird-read.js`, gated by `TINYBIRD_READ_ENABLED`, unset→null→HogQL fallback, proven no-op). `AI_ATTRIBUTION_VISITOR_BATCH_SIZE` 100→2500 (byte-math: ~45.5 bytes/UUID, 128KB cap → ~2,800-UUID ceiling; common case <~2,700 converters collapses N+1 to ONE call). `pageviews_by_visitors.pipe` gained `page_size`/`page_offset` params (default 5000 = preserves the old pagination bound — did NOT delete the safety cap). Read-only parity PASS on the cc-4d fixture (P/Q/R/S map + per-visitor aggregate, byte-identical to Phase-4d known-correct). **Two things still unverified, deploy/flag-gated:** the app CLIENT path itself (curl proved the pipe accepts the array shape; did not prove `tinybird-read.js`'s Node code constructs it correctly with the flag on — all tests so far ran the HogQL fallback), and the cost number (gating site has only 12 converters, so round-trips are 1→1 unchanged there; the win only shows >100 converters; needs `pipe_stats_rt` post-flag).
>   - **`nightly:565` (the same N+1 pattern in the nightly job) — STOP, NOT rewritten, confirmed money-rail coupling.** `processConversion`'s `:565` touchpoints query feeds `buildSubscriptionIdentitySeed({touchpoints, record})` at `:776` — `touchpoints.length===0` gates whether a `subscription_identity` seed WRITES, and `record.first_touch_*` (derived from those touchpoints) denormalizes into `subscription_revenue` (the actual money rows, `insertSubscriptionRevenue` `:795-837`). The query row-set is a write-path input, not just a downstream read — a "same aggregate result" rewrite bar would not catch a divergence in the `length===0` branch. Re-scoped as a **joint Phase 6/Phase 7 item**, validated at money-rail (write-path byte-identical) bar, not attempted solo.
>   - **Consolidation audit done — corrects the original "4 clusters" framing to 2 real ones.** Only `analytics.js` ×5 (`summary`/`sources_ai`/`sources_ref`/`browsers`/`os`, byte-identical, differ only by NODE name + default LIMIT → 5→1) and `alerts.js`↔`integrations.js` ×4 (byte-identical cross-file → 4-shared) are genuine duplicates. `setup-doctor`'s "5-fanout" and `integrations`'s "10-fanout" were mislabeled — they're Promise.all batches of *distinct* single-purpose queries (fanout ≠ duplication), not dup clusters; only 4 of the 10 `integ_*` overlap with cluster 2, the other 6 are unique. Both real merges **DEFERRED** — require app-caller changes (rename call sites, pass `limit_val` explicitly, repoint 2 route files), so merge only after the app is wired to read Tinybird (post app-path verification), not a blind pipe merge now. Plan documented in `tinybird/PHASE6_CONSOLIDATION_PLAN.md`.
>   - **`edge_domains`/`edge_ai_no_utm`/`edge_utm_no_ai` — faithful port confirmed, no fix.** These have no timestamp predicate, but live HogQL (`events.js:296-345`) is *also* unbounded — adding a bound would break parity, not fix cost. Documented the accepted cost characteristic (`rows_read=45` = full tenant history, scales with lifetime volume via the new `tb --cloud endpoint data <pipe> --site_id ...` per-endpoint telemetry tool) and flagged a caller-side date param for when `events.js` is wired, not fixed now.
> - **`flexible_report:2457` parity — BLOCKED on a founder-owned investigation, explicitly NOT a confirmed bug, NOT a migration blocker, NOT an agent fix.** This is a 6th HIGH-complexity attribution query Phase 4 never diffed — `attribution-engine.js`'s `getFlexibleReport` independently re-implements windowed touch-attribution self-joins inline (`windowJoin`, `:2236-2255`), a separate parallel path from the named `lastTouchAttribution`/`getMultiTouchAttributionLive` functions Phase 4 verified. Trigger: `isTouchModel` (first/last_touch + non_direct twins) AND `hasAttributionWindow` (set, numeric, !='ltv') AND `groupBy` ∈ {source, medium, campaign, keyword, referrer_domain}. A cc-4a/cc-4c fixture exists that exercises this combo, but the reconstructed live HogQL reference query **OOM'd on PostHog 469905 even on a near-empty dataset (3,059 total events)** — ruled out data-scale, isolated to a minimal 2-row repro of the same join shape (relational operators `>=`/`<=` inside a `LEFT JOIN ON` clause, which the finding claims HogQL disallows, forcing full materialization). **Orchestrator skepticism, unresolved:** the error (`MaxToolRetryableError: max memory limit`) may be the MCP tool's own memory ceiling, not PostHog's query engine — not ruled out; the query was a *reconstruction*, not the actual `getFlexibleReport` code path — could carry a reconstruction defect; "has this touch-model+numeric-window combo ever fired in prod" is unanswered and is the whole question (if the UI never sends it, the path is latent, not active). **Founder action before any P0/P1 label:** check prod error logs for OOM on this endpoint + whether the UI exposes the combo; run the ACTUAL prod endpoint (not a reconstruction) with it. Same `pv.timestamp<=cv.timestamp`-in-ON pattern also present (code-inspection only, not executed) in `touchpoints_per_conversion` (`:1935-1945`), `days_to_convert`, and the legacy `linear` path — same file, same unresolved status.
> - **Token security:** `st_endpoint_read`'s value was accidentally exposed during this session (a `tb token copy`/command-substitution mishap printed it in plaintext). `WORKSPACE:READ_ALL`, staging-only — low practical risk, but real. **Founder chose to skip rotation for now** — carried as pending, alongside the already-standing `dual_write_append` + admin token rotation (3 tokens total pending). `tb --cloud token copy <name>` does **not** reliably copy-to-clipboard/stdout on CLI v4.6.4 (misparses as a bad subcommand) — use the Tinybird web UI to copy full token values, or paste inline into curl; never via `$(...)` substitution on this CLI version.
> - **Open backlog, carried forward + extended (none block Phase 5/6, all block CUTOVER):**
>   1. **`flexible_report:2457` parity** — blocked on founder investigation, see above.
>   2. **SEO-revenue organic fixture gap** — founder must seed a dedicated organic (referrer=google, no gclid, real conversion) fixture; zero coverage today.
>   3. **NULL-vs-DEFAULT ingest ambiguity / Nullable-vs-DEFAULT DDL decision** — deferred to Phase 7/8 data-model review, see above.
>   4. **Dual-write adapter DATA divergence, new this session:** `cc-verify-proxye-20260630a` has `browser_name='webkit'` in Tinybird but `None` in live PostHog — a typed-column VALUE mismatch (the adapter stored something different than PostHog for the same event, likely UA-parsing/enrichment divergence on the `/sp/e` proxy route). Write-side reconciliation item, not a read-pipe bug — flagging, not fixed.
>   5. **Merged-identity coverage** (`visitor_id != distinct_id`) — unchanged from prior entry, still no fixture across any Phase 4/5 batch.
>   6. **Group-by aggregation-layer parity** — unchanged from prior entry, still not diffed end-to-end.
>   7. **`first_touch` + the 2 `*_non_direct` variants** — cutover-gate, restated, unchanged.
>   8. **retry-transport-at-boot wiring** — 429/5xx not retried on the dual-write path; separate from the resolved dual-write artifact question.
>   9. **`nightly:565` money-rail-coupled N+1 rewrite** — re-scoped to joint Phase 6/7, see above.
>   10. **`analytics.js` ×5 / `alerts↔integrations` ×4 pipe consolidation** — deferred until app is wired to read Tinybird, plan in `tinybird/PHASE6_CONSOLIDATION_PLAN.md`.
>   11. **Prod read-token model** — staging's `WORKSPACE:READ_ALL` is broader than intended; Phase 10 should design per-tenant JWTs for the real prod read rail.
> - **Artifacts committed this session** (`84504fa`→`d06ead7`, all pushed): 54 endpoint pipes, `api/lib/tinybird-read.js`, `attribution-engine.js` (aiplatform rewrite), `dashboard/src/pages/SEORevenue.jsx` (truth-gate fix), 2 corrected Phase-4 pipes, `tinybird/PHASE6_CONSOLIDATION_PLAN.md`. Deployment #4 is live in ST_Staging and matches the committed state (param-typing fixes from `deploy --check` are committed as `056c303`, not stranded).
> - **Immediate next step, not started this session:** **app-path verification** for `aiplatform:505` — set Railway STAGING env (`TINYBIRD_HOST`, `TINYBIRD_READ_TOKEN`, `TINYBIRD_READ_ENABLED=on`), restart, hit the `ai_platforms` report, diff the result against the known cc-4d aggregate through the LIVE Tinybird client (not the HogQL fallback), then pull `pipe_stats_rt` for real cost numbers. Open decision, not resolved this session: who sets the Railway env (founder by hand vs Antigravity via Railway MCP). Phase 7 (money rail: `nightly:565` + refunds + GDPR erasure + quarantine alarm) deliberately **not started** this session — highest-risk phase, reserved for a fresh session with full attention.
>
> **Handoff:** Session TINYBIRD-PHASE4-COMPLETE — Phase 4 Row-Pull Migration (4a–4d) Signed Off — **PASS.**
> - **All 4 Phase-4 checkpoints signed off**, each independently orchestrator-verified via PostHog 469905, not just CC-asserted: **4a** (`time_decay` + Pattern-B touchpoint-set parity), **4b** (`linear`/`u_shaped`/`w_shaped` on the same Pattern-B pair), **4c** (`last_touch`, Pattern A correlated join), **4d** (`ai_platforms`, Pattern C IN-list pull). Builds on the `TINYBIRD-2C-4A-4B` entry below (pageview dual-write gap close, +339ms interval-comparison rule) — this entry covers 4c and 4d specifically.
> - **4c — `last_touch` shipped as A-3 (per-field independent `ASOF LEFT JOIN`s), not a single-row pick.** A single `ASOF JOIN` (one row, all its fields together) passed the planned exact-timestamp tie-break test but **failed silently on a different, undesigned-for case**: HogQL's live `lastTouchAttribution` computes `argMax(column, timestamp)` **independently per column** (`utm_source`, `utm_medium`, `ai_source`), and ClickHouse's `argMax` skips rows where the aggregated column is NULL — so the live model can (and, on the 4c fixture, did) credit `utm_source` from one touchpoint and `ai_source` from a different, more recent one when the true last touch had a null in one field. Confirmed against real fired data (visitor Y: HogQL returned `facebook` + `ChatGPT` from two different touches), not assumed. Fixed by building 3 independent `ASOF JOIN`s, one per field, each pre-filtered to non-null rows for that field — reproduces HogQL's actual behavior (including the null-skip quirk) exactly, 3/3 on the fixture. This is a **faithful parity port, not a correctness fix** — replicating HogQL as-is is the explicit Phase 4 goal; whether the underlying model behavior (crediting fields from different touchpoints) is itself correct is a separate, **deferred product-semantics decision** (see backlog below). **Not yet reflected as a numbered section in `GATE3_RECONCILIATION_CONTRACT.md`** as of this entry — checked the committed file directly, it currently ends at §6; citing it as "§7" would be asserting something not yet true. Also confirmed in the same pass: A-2 (a literal nested-`LEFT JOIN`+`argMax` port of HogQL's SQL) is **structurally rejected by Tinybird outright** (`Unsupported JOIN ON conditions` on the mixed equality+inequality `ON` clause) — `ASOF JOIN` is mandatory, not a preference.
> - **4d — `ai_platforms` shipped as Pattern C with the chunk-at-100 visitor-batching workaround KEPT, not removed.** Checked first whether Pattern A's null-skip risk applied here: it doesn't — `selectAiTouchForConversion` is a JS backward array scan returning one whole touchpoint object, not a SQL aggregate, confirmed by reading the function. The open question was instead whether Tinybird's `{{Array(...)}}` template param could replace HogQL's chunk-at-100/page-at-5000 IN-list workaround. Could not test the live template mechanism directly (would require pushing a pipe, which is out of scope/authorization) — found Tinybird's documented **128KB SQL-length limit** instead and measured a 2,000-realistic-UUID array at ~91KB from the array alone. Kept the chunking (100 UUIDs ≈ 4KB, safely under the limit) rather than conclude "unneeded" from the fixture's small visitor count, which would have been a false-green.
> - **Tier confirmed Enterprise** (`tb --cloud workspace current`, all 3 workspaces) during 4c — corrected `SCOPE_v3.md` §0.3, which asserted "org-wide Free" with 1k req/day / 10GB / 0.25vCPU limits. That framing no longer drives any Phase-4 design decision (the 4d chunking decision above is driven by the 128KB SQL-length limit instead, which is plan-independent).
> - **Scope limit, stated on both 4c and 4d, not just in their plans:** both are validated for **single-identity visitors only** (`distinct_id == visitor_id`). Neither fixture exercises a merged identity (`server-events.js`'s `user_id`-merge path, `normalize.js:251-253`, can in principle diverge the two). Deferred to the `$identify` backlog item below.
> - **Second scope limit, gap-closed in this addition — was only in the `.pipe` docstrings, not surfaced here before:** both new pipes' join/IN-list key is `visitor_id`, while HogQL's live path joins/filters on `distinct_id` (`lastTouchAttribution`'s `pv.distinct_id = e_inner.distinct_id`, `engine:454`'s `conversions.map(c => c.distinct_id)` for the ai_platforms IN-list). For single-identity visitors the two are equal, so neither fixture can distinguish this choice from the alternative — it's a real, separate unresolved design question on top of (not the same as) the general merged-identity gap above, and is deferred to the same `$identify` item.
> - **Open backlog, carried forward + extended:**
>   1. **Merged-identity coverage** (`visitor_id != distinct_id`) across `last_touch` and `ai_platforms` — deferred to the `$identify` / identity-reconciliation item (same item noted in the prior handoff entry, now explicitly covers 4c/4d too, not just the dual-write gap).
>   2. **Group-by aggregation-layer parity** — all of 4a–4d proved row-level touchpoint-set / picked-touchpoint / credited-platform parity only; the dashboard-facing `groupBy`-aggregated API response hasn't been diffed end-to-end. Needs a multi-touch-shared-source fixture (carried from the prior entry, unchanged).
>   3. **`first_touch` + the 2 `*_non_direct` variants** — cutover-gate, never in Phase-4 scope, restated for completeness.
>   4. **Product-semantics decision, new:** `last_touch`'s per-column field-fusion (4c finding above) is replicated faithfully for migration parity, but whether crediting fields from two different real touchpoints as if they were one is the *correct* attribution behavior is unresolved — explicitly deferred to a **post-cutover** decision, not a Phase-4 blocker.
>   5. **`normalize.js` reserved-bag hardening** and **`$identify` dual-write gap itself** — both carried unchanged from the prior entry.
> - **Server-side surfaces** (the managed-proxy ingestion path and the CAPI/server-side-conversions scope) are **documented for sales/reference purposes only** in this cycle — not built, tested, or claimed working as part of 4a–4d.
> - **Artifacts committed this session:** `tinybird/pipes/last_touch_by_site.pipe`, `tinybird/pipes/pageviews_by_visitors.pipe`, `tinybird/PHASE4_4C_PLAN.md`, `tinybird/PHASE4_4D_PLAN.md`, `tinybird/SCOPE_v3.md` (§0.3 correction). `tinybird/GATE3_RECONCILIATION_CONTRACT.md` was placed and committed separately (already in the tree as of this entry). None of the new `.pipe` files have been pushed/deployed (`tb push`) — validated read-only via `tb --cloud sql` only, per the standing no-push rule throughout Phase 4.
> - **Phase 5 — starting point (citations corrected, not copied as given):** Phase 5 in `SCOPE_v3.md` §13's phase table is **`Mechanical endpoint ports (~75) | low`** — the *project phase number* (5), not a document section number. The descriptive content behind that count — "~75 mechanical reads → site-scoped endpoint pipes (`WHERE site_id = {{String(site_id)}}` backend-injected)" — actually lives in document **§4 "Read design"**, not §5 (§5 is "MV vs Copy Pipe split," a different, already-largely-executed topic — conflating the two would mis-cite the spec). The **template for these ports** is [`tinybird/pipes/bench_conversions_by_site.pipe`](tinybird/pipes/bench_conversions_by_site.pipe), per its own `DESCRIPTION` field ("Benchmark + template endpoint... `site_id` filter must prune via the `(site_id, timestamp)` sorting key") — SCOPE_v3.md itself doesn't name this pipe anywhere, so that's the pipe file as the source of truth, not the spec doc. **Deferred Phase-4 tail that may sequence before or alongside Phase 5, not necessarily blocking it:** merged-identity coverage and group-by aggregation-layer parity (backlog items 1 and 2 above) — neither is a hard prerequisite for porting mechanical (non-attribution-model) reads, but both should be resolved before Phase 5's output is treated as load-bearing for anything attribution-shaped.
> - **Process note for future sessions:** `tinybird/GATE3_RECONCILIATION_CONTRACT.md` has **one canonical copy, in-tree**. Amend it in place; never create a second/parallel contract doc elsewhere. This session hit real, repeated doc-drift (the `8bafb4a`/"Phase 2d sign-off" citation that turned out not to exist anywhere in this repo, and the "contract §7" citation that didn't exist in the committed contract at the time it was referenced) — both were caught only by directly reading the actual tracked file before citing it, not by trusting the citation as given. Do the same going forward: read the in-tree file before citing a section number from it.
>
> **Handoff:** Session TINYBIRD-2C-4A-4B — Pageview Dual-Write Gap Close + Phase 4a/4b Touchpoint-Set Parity — **PASS.**
> - **HEAD:** `af82218` on `claude/tinybird-phase1-events-schema` (pushed). Note: a repo-wide search found no prior entry in this file (or `DEVELOPER_CONTEXT.md`/`NEXT_SESSION_PROMPT.md`/`PROGRESS.md`) referencing Tinybird, `8bafb4a`, or a "Phase 2d dual-write proven end-to-end" sign-off — this is the first Tinybird-workstream entry in this file, not a correction of an existing one.
> - **Pageview dual-write gap found and closed:** Live touchpoint-parity work surfaced that PostHog (469905) had pageviews for the gating site (`de200000-...441111`) while Tinybird had zero — the `$pageview` ingestion path (`/api/track`, `/sp/e`, `/sp/pixel.gif`) never called `dualWriteEvent`, only `$conversion` producers did. Fixed in `af82218`: wired all three routes behind the existing `TINYBIRD_DUAL_WRITE` flag, mirroring the established conversion-dual-write pattern. Two real bugs caught and fixed during implementation, not just wiring: (1) `distinctId` was computed inline (`anonymous_id || uuidv4()`) at each `ph.capture` call site — naively copying that to `dualWriteEvent` would call `uuidv4()` twice and silently break visitor stitching between stores for anonymous pageviews; fixed by hoisting to a single shared const per handler. (2) `/sp/pixel.gif`'s properties bag carries `event_type: 'pixel'` (a PostHog-only analytics label) which collides with Tinybird's canonical `event_type` discriminator (`normalizeEvent` prefers a bag `event_type` over the wrapper `event`) — would have silently mislabeled every pixel-fallback pageview row; fixed by renaming to `tracking_method: 'pixel'` for the dual-write payload only (PostHog side untouched). Live-verified (§5): fired 3 fresh anonymous events through all 3 routes on staging, confirmed byte-identical `distinct_id` across both stores, correct `event_type`/`tracking_method`, zero quarantine.
> - **+339ms timestamp-correction finding:** PostHog applies an ingestion-side timestamp adjustment to backdated/historical events — confirmed it is NOT a `posthog-node` SDK behavior (read the installed SDK source directly: `prepareMessage()` passes the supplied `timestamp` through unaltered, no `sentAt`/offset logic exists anywhere in the package). For a 15-event backdated fixture, all 15 timestamps shifted a uniform +339ms in PostHog relative to Tinybird's unaltered stored value; exhaustively checked every candidate PostHog field (`$sent_at`, `server_timestamp`, `created_at`, the full `events` column list) — none preserve the original client-sent value. Production near-real-time events get negligible correction since there's little client/server clock gap to correct for. **Reconciliation rule, now load-bearing:** compare timestamp **intervals** (within-journey deltas: touch-to-touch, conversion-to-last-touch), not absolute timestamps — the uniform offset cancels out of every delta, which is what attribution math (`calculateAttribution`, especially `time_decay`'s `0.5^(daysBack/7)`) actually consumes. Full detail belongs in `tinybird/GATE3_RECONCILIATION_CONTRACT.md` — **not yet present in this repo as of this entry** (see artifact note below).
> - **Phase 4 checkpoint status:** **4a SIGNED OFF** — `time_decay` + Pattern-B (`conversions_by_site`/`pageviews_windowed_by_site`) touchpoint-set parity: 15/15 rows field-identity-exact, 18/18 within-journey interval deltas exact (0ms), independently orchestrator-verified via PostHog 469905. **4b SIGNED OFF** — `linear`/`u_shaped`/`w_shaped` verified on the same Pattern-B pair and fixture (same row-pull, only the JS allocation branch differs; no new firing needed). **Next:** 4c (`last_touch`, Pattern A correlated join) and 4d (`ai_platforms`, Pattern C IN-list pull) — not yet started.
> - **Open backlog (flagged, not yet scheduled):**
>   1. `normalize.js` reserved-bag hardening — add `event_type` to a forbidden-from-customer-bag set; the live `/sp/c` conversion dual-write rail is exposed today to the same collision class found in `/sp/pixel.gif` if a customer ever sends a literal `event_type` property.
>   2. `$identify` dual-write gap — `identify.js` never calls `dualWriteEvent`; pre-cutover parity item (anonymous→known identity merges won't replicate to Tinybird until wired).
>   3. Group-by aggregation-layer parity — 4a/4b proved row-level touchpoint-set + allocation-fraction parity only; the actual dashboard-facing `groupBy`-aggregated API response hasn't been diffed end-to-end. Needs a multi-touch-shared-source fixture (several visitors converging on the same channel) later in Phase 4.
>   4. Cross-visitor-bleed coverage — deferred to 4c by design, since Pattern A (`last_touch`'s correlated join) does visitor-matching inside SQL, unlike Pattern B's shared, unscoped flat pull (visitor-matching happens in shared JS there, unaffected by the row-pull swap).
> - **Artifacts:** `tinybird/PHASE4_BUILD_PLAN.md`, `tinybird/PHASE4_4A_FIELD_MAPPING.md`, `tinybird/PHASE4_4A_FIXTURE_PLAN.md`, `tinybird/PHASE2C_PAGEVIEW_DUALWRITE_PLAN.md`, `tinybird/pipes/conversions_by_site.pipe`, `tinybird/pipes/pageviews_windowed_by_site.pipe`, `tinybird/tools/phase4_touchpoint_diff.js` — previously parked uncommitted, committed this session now that 4a/4b are proven. **`tinybird/GATE3_RECONCILIATION_CONTRACT.md` does not exist anywhere in this repo** (verified by repo-wide search) and was not committed — founder needs to place it first.
>
...
> - **Added Clock Skew & Source Prioritization Regression Tests**: Expanded `api/tests/analytics-sources-join-ms.test.js` to run two integration tests: (1) verifying ChatGPT revenue mapping despite a 123ms offset, and (2) verifying source prioritization when two pageviews occur within 5s (AI referral vs Direct/internal) and the conversion timestamp is closer to the Direct pageview. Verified both tests go GREEN.
> - **Redid Core-Product Audit & Verification Map**: Updated `core_product_audit_ci_hardening_map.md` with a detailed per-feature verification table mapping test existence, seed reconciliation, and whether it runs in CI (offline/gated) for all core paths, specifically highlighting untested paths (GSC, Reverse Proxy, Leads Journey).
> - **Hardened CI Blueprint**: Rejected HogQL mock/emulators in `core_product_audit_ci_hardening_map.md` to prevent false greens on same-second ordering or timezone coercion, proposing Dockerized ClickHouse/Postgres service containers or gated integration tests.
>
> **Handoff:** Session 140P-RB-DIM-2 — Dimension Columns Reprocess & Timezone Reconciliation — **PASS.**
> - **Seeder Baseline Date Offset:** Adjusted baseline `now` date in `seed-diagnostic.mjs` (stored in the scratch folder) to `2026-06-22T01:00:00.000Z` to correctly align visitor conversions with timezone reconciliation test assertions ($198 on June 21 and $1,110 overall).
> - **Ingested PostHog Event Propagation:** Reprocessed site `de400000-babe-41d4-a716-446655440000` via the nightly-attribution job to calculate and insert all 38 conversions into the Supabase database.
> - **Second-Level Precision Alignment:** Refactored timestamp matching logic in `api/routes/analytics.js` (for both `referrer` and `ai_source` tab paths) to use second-level resolution (rounding to `Math.floor(timestamp / 1000)`). This prevents precision-loss mismatches between PostHog's UUID v7 ingestion milliseconds and Supabase's `000` millisecond pageview timestamps, successfully reconciling AI Search (ChatGPT/Claude) revenues.
> - **Passed Unit & Integration Tests:** Confirmed all 20 tests in `npm run qa:attribution:unit` pass successfully. Verified that `git diff --check` and `npm run build` inside `dashboard` compile with zero errors.
>
> **Handoff:** Session 140P-RB-FIX-4 — Report Builder Disable Unsupported Dimensions — **PASS.**
> - **Dimension UI Picker Removal:** Removed `country`, `device`, `browser`, and `landing_page` dimensions from the `DIMENSIONS` configuration list in `ReportBuilder.jsx`, hiding them from the dimension picker dropdown.
> - **Preset Templates Update:** Updated the two preset templates (`saas_signups` and `lead_forms`) that originally grouped by `landing_page` to instead group by `campaign` (and updated their display names/descriptions accordingly) to prevent frontend-triggered 400 errors.
> - **Server-side Gate Validation:** Removed the 4 unsupported dimensions from `ALLOWED_GROUPS` in both `api/routes/attribution.js` and `api/lib/report-config-validation.js`. Added a check for `UNSUPPORTED_GROUPS` at the very beginning of the dimension validation blocks in both files to reject requests with `400 Bad Request` and the specific message `"Dimension not supported: <dimension>"`.
> - **Verified No Regression:** Ran all 3 test suites: `report-builder-leads.test.js` (8/8 passing), `timezone-reconciliation.test.js` (4/4 passing), and `attribution.test.js` (9/9 passing). Verified with a scratch query script that the API returns the specific 400 error message for all 4 disabled dimensions on both `group_by` and `group_by2` parameters, while supported dimensions (like `campaign`) continue to return successful 200 responses.
> - **Production Build compiles:** Confirmed dashboard frontend production build builds cleanly without errors.
>
