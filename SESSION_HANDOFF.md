...
>
> **Handoff:** Session 140G-4 — Pageview Limit Enforcement / Usage Counter Architecture — **PASS — plan-based monthly pageview limits are locally implemented and unit-tested across known ingestion points; staging/production verification remains pending. Paid beta remains blocked by PostHog retention/purging and the remaining open release gates, including paid billing portal verification, production billing verification, production env/secrets verification, tenant isolation, privacy/deletion, observability, install QA, and docs truth audit.**
> - **Pageview Limit Enforcement**: Implemented monthly pageview limit checking logic using a new `claimPageviewUsage` helper in `api/lib/pageview-limits.js` utilizing the atomic `claim_site_pageview_usage(...)` PostgreSQL RPC with row-level locking. Integrated checks across standard tracker ingestion (`POST /api/track`), proxied browser ingestion (`POST /sp/e` and `GET /sp/pixel.gif`), and the legacy collect route (`POST /api/analytics/collect`). Checks are late-gated (run after bot filtering and path exclusions to avoid consuming quota for discarded events). Bypassed checks on custom events, conversions, and outbound clicks. Deleted `checkTierLimit` from the conversion route stack (`index.js`). Fails open on database/query errors.
> - **Pageview Limit Tests**: Added comprehensive unit and integration tests under `api/tests/billing-middleware.test.js` covering helper checks (free plan below/at limits, override limits, inactive/archived plans, UTC YYYY-MM month format), and route integration tests (below/at limit, custom events, bot filtering, path exclusions, and fail-open behaviors). All tests pass. QA: docs/qa/pageview_limit_enforcement_140G-4.md.
