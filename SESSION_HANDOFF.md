...
>
> **Handoff:** Session 139N-4E — Fix Supabase Auth StorageKey Env Collision — **PASS**.
> - **StorageKey Env Collision Fix:** Modified `dashboard/src/lib/supabase.js` to dynamically extract the Supabase project reference from `VITE_SUPABASE_URL` and use it to construct the localStorage `storageKey` (`sb-${projectRef}-auth-token`). If parsing fails, it defaults to a neutral `sb-sourcetrack-auth-token` (never hardcodes the production project ref).
> - **Staging E2E State:** Staging password reset email E2E remains **PASS** (139N-4D, after config fix). Dynamic storageKey resolves correctly on local static/build checks. Post-deploy browser verification on the deployed staging environment is pending to verify localStorage uses the new `sb-nrsvpwzekfrdrzkoecfk-auth-token` key namespace.
> - **Production password reset remains unverified.** **Paid beta remains blocked** until production/canonical-domain auth and remaining P0 blockers are verified. No app code changes outside `supabase.js` were made; no secrets or tokens printed; no commit. QA: docs/qa/auth_storage_key_env_collision_139N4E.md.
>
