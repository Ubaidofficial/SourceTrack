...
>
> **Handoff:** Session 139N-4D — Staging Password Reset Email E2E — **FAIL — Supabase Auth redirect URL misconfigured (P0 auth blocker)**.
> - **Works up to delivery:** reset request submitted PASS ("Check your inbox…"); Supabase recovery **email delivered PASS** (staging, `imubaid93@gmail.com`).
> - **BUG:** the recovery link redirects to **`http://localhost:3000/`** with a recovery URL-hash fragment (token values redacted) (dead → "Cannot GET /") instead of the deployed `/reset-password`, so password update / login-after-reset could not proceed.
> - **Root cause = CONFIG, not code:** app already passes `redirectTo=${origin}/reset-password` (ForgotPassword.jsx) and consumes the recovery hash (ResetPassword.jsx + `detectSessionInUrl`), but Supabase ignored the redirect (deployed URL not in the **Auth Redirect URLs allowlist**) and fell back to **Site URL = `http://localhost:3000`** (dev default).
> - **FIX (Supabase Dashboard → Auth → URL Configuration; operator, no app deploy — staging & production are SEPARATE Supabase projects, do NOT mix their URLs):** on the **staging** project (`nrsvpwzekfrdrzkoecfk`) set **Site URL** = `https://sourcetrack-dashboard-staging.up.railway.app` and **Redirect URLs** = staging `/reset-password`,`/auth/callback`,`/login` + `https://sourcetrack-dashboard-staging.up.railway.app/**` + `http://localhost:3000/**` (later `https://staging.sourcetrack.ai/**`). Production URLs (`https://www.sourcetrack.ai/**`) belong **only** in the separate production Supabase project. Then re-run §10–§13.
> - **Secondary latent bug (flagged, not changed):** `dashboard/src/lib/supabase.js` hardcodes `storageKey` to the **prod** ref (`sb-zxjjjsipafojhzkkumvh-auth-token`) while staging uses ref `nrsvpwzekfrdrzkoecfk` → multi-env session collision risk; recommend per-env storageKey.
> - **Production:** still UNVERIFIED. **Paid beta:** still BLOCKED. No app code changed; no reset link/password/token printed; no commit. QA: docs/qa/staging_password_reset_email_e2e_139N4D.md.
>
...
>
> **Next Task:** Apply staging Supabase Auth URL configuration, then rerun **139N-4D** to complete reset link → /reset-password → password update → login → dashboard/onboarding routing. On the STAGING Supabase project (`nrsvpwzekfrdrzkoecfk`) set **Site URL** = `https://sourcetrack-dashboard-staging.up.railway.app` and **Redirect URLs** = `https://sourcetrack-dashboard-staging.up.railway.app/reset-password`, `…/auth/callback`, `…/login`, `https://sourcetrack-dashboard-staging.up.railway.app/**`, `http://localhost:3000/**` (later branded staging: `https://staging.sourcetrack.ai/**`). Production URLs (`https://www.sourcetrack.ai/**`) belong ONLY in the separate production Supabase project — **do NOT mix staging and production Auth redirect URLs across Supabase projects.** Production auth remains **UNVERIFIED**, but the immediate unblock is the staging Supabase Auth URL config (139N-4C is closed as a production/staging browser QA artifact). Other open items unchanged: Fix billing status validateSiteKey database select (P0); Stripe browser billing UI verification (139J leftover); align staging GOOGLE_GSC_REDIRECT_URI; Supabase staging restore drill + PITR decision; P1 attribution-model test fixtures. Paid beta is NOT ready until these P0/P1 conditions (incl. verified password reset) are met.
>
