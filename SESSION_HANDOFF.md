...
>
> **Handoff:** Session 139L — Browser/Staging Verification is **PASS** (staging deploy `cee2954`). Real browser + API QA confirmed the Terms/Privacy checkout gate end-to-end: `/billing` shows the acknowledgment checkbox above the plans grid with all paid upgrade buttons disabled until checked (links to `/terms` + `/privacy`, new tab); `/terms` and `/privacy` load with no 404 and no false compliance claims; checking/unchecking toggles the buttons; with the box checked, Upgrade redirects to **Stripe Checkout test mode** (`cs_test_`); `POST /api/billing/create-checkout` returns **400 "Terms and Privacy acknowledgement is required before checkout."** when `accepted_terms` is missing/false and **200** (normal Stripe path) when `true`; `POST /api/billing/portal` is **not** terms-gated (returns "No Stripe customer — subscribe first" on the free account, creates no checkout). Limitation: acceptance is enforced as a request gate, not persisted. This closes the **Terms/Privacy payment-disclosure gate only** — paid beta is **not** marked ready (other P0s remain). QA: [beta_terms_privacy_disclosure_qa_139L.md](docs/qa/beta_terms_privacy_disclosure_qa_139L.md) (Browser/Staging Verification Addendum).
>
...
>
> **Next Task:** Operator to select the next pre-paid-beta P0 (139L Terms/Privacy gate is browser-verified PASS and closed). Open candidates: Stripe browser billing UI + billing status endpoint (139J leftover); Supabase staging restore drill + PITR decision; production env secrets incl. ST_IP_RESOLVER_MODE=railway. Paid beta is NOT ready until these P0 conditions are met.
>
