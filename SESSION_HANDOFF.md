...
> - **Initial run FAILED (real, documented):** reset request submitted PASS; Supabase recovery **email delivered PASS** (staging, `imubaid93@gmail.com`); but the recovery link redirected to **`http://localhost:3000/`** (dead → "Cannot GET /") instead of the deployed `/reset-password`.
> - **Root cause = provider CONFIG, not app code:** app already passes `redirectTo=${origin}/reset-password` (ForgotPassword.jsx) and consumes the recovery hash (ResetPassword.jsx + `detectSessionInUrl`); Supabase ignored the redirect (deployed URL not allowlisted) and fell back to **Site URL = `http://localhost:3000`** (dev default).
> - **Config fix (operator, in-console, no app deploy — staging & production are SEPARATE Supabase projects, do NOT mix their URLs):** on the **staging** project (`nrsvpwzekfrdrzkoecfk`) Site URL = `https://sourcetrack-dashboard-staging.up.railway.app`; Redirect URLs include `/reset-password`, `/auth/callback`, `/login`, the staging wildcard, and the localhost dev wildcard. Production URLs (`https://www.sourcetrack.ai/**`) stay **only** in the separate production project.
> - **Final verification (operator MANUAL, post-fix):** fresh reset email PASS → link landed on staging `/reset-password` PASS → password update PASS → login after reset PASS → staging `/dashboard` loaded PASS (screenshot: staging dashboard URL).
> - **Secondary latent bug (flagged, not changed):** `dashboard/src/lib/supabase.js` hardcodes `storageKey` to the **prod** ref (`sb-zxjjjsipafojhzkkumvh-auth-token`) while staging uses ref `nrsvpwzekfrdrzkoecfk` → needs environment/project-specific storageKey.
> - **Production password reset remains unverified.** **Paid beta remains blocked** until production/canonical-domain auth and remaining P0 blockers are verified. No app code changed; no reset link/password/token printed; no commit. QA: docs/qa/staging_password_reset_email_e2e_139N4D.md.
>
...
>
> **Next Task:** Verify **PRODUCTION / canonical-domain** password reset E2E. Staging password reset is now **PASS** (139N-4D, after the Supabase Auth URL config fix); production remains **UNVERIFIED**. On the **production** Supabase project (separate from staging) confirm the same per-project Auth URL config — Site URL = the production dashboard/canonical domain, Redirect URLs = production `/reset-password`, `/auth/callback`, `/login` + `https://www.sourcetrack.ai/**` (and the production Railway host if still used) — keeping staging and production redirect URLs **strictly separate** (do NOT mix). Then run the production reset email → link → `/reset-password` → password update → login → dashboard chain once production browser inspection is unblockable. Other open items unchanged: Fix billing status validateSiteKey database select (P0); Stripe browser billing UI verification (139J leftover); align staging GOOGLE_GSC_REDIRECT_URI; Supabase staging restore drill + PITR decision; P1 attribution-model test fixtures; secondary follow-up = make `dashboard/src/lib/supabase.js` auth `storageKey` environment/project-specific. Paid beta is NOT ready until these P0/P1 conditions (incl. verified production password reset) are met.
>
