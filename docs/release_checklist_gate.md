...
- [ ] **Data Deletion & Privacy Basics**: Ensure manual and automated user deletion routines purge associated personal data correctly from the database.<br />*Status:* **PARTIAL (Supabase DB deletion code path audited; PostHog event retention, proxy sanitization, and live deletion verification remain BLOCKED)**. Individual visitor erasure code path deletes Supabase attribution rows and stitched identity links, and sends a best-effort delete command to PostHog. Account deletion code path cascades to purge Supabase workspace data, but leaves historical raw events inside the PostHog project. Proxy routes currently bypass PII sanitization. Manual console/API limits and operator runbooks remain pending. See [posthog_retention_deletion_audit_140G-5.md](qa/posthog_retention_deletion_audit_140G-5.md).
