...
- [ ] **Attribution Model Deterministic Test Fixtures**: Add deterministic test scenarios for the core attribution models (first-touch, last-touch, linear, U-shaped, W-shaped, time-decay) to protect against regressions.<br />*Status:* **PARTIAL** — deterministic `calculateAttribution` model math is covered by Node `node:test` unit tests. Real end-to-end revenue attribution remains blocked by staging schema, identity linkage, seeded journeys, and webhook/E2E verification.
