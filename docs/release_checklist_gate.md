...
- [ ] **Billing/Limits Enforcement Audit**: Audit all billing/limit checking middleware to ensure free and active-tier users cannot exceed their monthly thresholds without payment.<br />*Status:* **PARTIAL (webhook leak and site limits fixed)**. Outbound webhook downgrade leak is **FIXED** (dispatch is skipped on downgraded/free tiers). Plan-based active site limits are **FIXED** and enforced on backend site creation (returns 402, verified by unit tests). However, plan limit enforcement remains incomplete and paid beta remains blocked because: (1) standard tracker pageview limits are bypassed; (2) conversion caps are not enforced; (3) PostHog retention is not purged. Added automated unit tests to `api/tests/billing-middleware.test.js` covering `checkTierLimit` middleware, `dispatchWebhook` plan enforcement, and `checkSiteCreationLimit` site limit check. See docs/qa/billing_limits_enforcement_audit_140G.md, docs/qa/webhook_downgrade_leak_fix_140G-1.md, and docs/qa/site_limit_enforcement_140G-2.md.
