...
- [ ] **Password Reset & Auth Flow Verification**: Implement and verify that forgot password, recovery link redirects, and password updates work correctly end-to-end. Configure and verify allowed redirect URLs and SMTP templates in the Supabase console.<br />*Status:* **BLOCKED / FAIL (config)**. Flow implemented & deployed (139N-4B, `3e41f58`); staging auth routes browser-verified PASS (139N-4C/4D). 139N-4D staging email E2E: reset request submitted PASS and the Supabase recovery **email delivered PASS**, but the recovery link **redirects to `http://localhost:3000/`** with a recovery URL-hash fragment (dead) instead of the deployed `/reset-password` → password update/login-after-reset blocked. **Root cause = Supabase Auth URL config, not code**: app already passes `redirectTo=${origin}/reset-password` and consumes the recovery hash, but the deployed URL is not in the **Auth Redirect URLs allowlist**, so Supabase falls back to **Site URL = `http://localhost:3000`** (dev default). **Required fix (Supabase Dashboard → Auth → URL Configuration — staging & production are SEPARATE Supabase projects; do NOT mix their URLs):** on the staging project (`nrsvpwzekfrdrzkoecfk`) set Site URL = `https://sourcetrack-dashboard-staging.up.railway.app` and Redirect URLs = staging `/reset-password`,`/auth/callback`,`/login` + `…staging…/**` + `http://localhost:3000/**`; production URLs (`https://www.sourcetrack.ai/**`) go ONLY in the production Supabase project. Secondary: `supabase.js` storageKey hardcoded to the prod ref while staging uses a different project. **Production reset remains unverified.** Not done until the full email→link→password-update→login chain passes on a deployed env and the Auth-URL config is confirmed.
