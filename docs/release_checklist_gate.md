...
- [ ] **Billing/Limits Enforcement Audit**: Audit all billing/limit checking middleware to ensure free and active-tier users cannot exceed their monthly thresholds without payment.<br />*Status:* **PARTIAL (audit performed & webhook leak fixed)**. Comprehensive code audit completed. Outbound webhook downgrade leak is **FIXED** (dispatch is skipped on downgraded/free tiers) and validated by unit tests. However, plan limit enforcement remains unfixed and paid beta remains blocked because: (1) standard tracker pageview limits are bypassed (due to counting from the empty Supabase `pageviews` table instead of PostHog); (2) conversion caps are not enforced; (3) site limits are not enforced; (4) PostHog retention is not purged. Added automated unit tests to `api/tests/billing-middleware.test.js` to test `checkTierLimit` middleware and `dispatchWebhook` plan enforcement behavior; note that these tests validate middleware routing, webhook plan checks, and database fallback logic, but do not validate end-to-end plan limit enforcement across PostHog ingestion. Detailed findings logged in `docs/qa/billing_limits_enforcement_audit_140G.md` and `docs/qa/webhook_downgrade_leak_fix_140G-1.md`.
