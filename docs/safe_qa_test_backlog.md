...
| **P0** | Upgrade production Supabase to paid plan and enable backups/PITR. | Production Supabase is currently on the Free plan, which disables daily scheduled backups and PITR. | Operator upgrades the production database to a paid tier and enables backups and PRIV/PITR. | **CRITICAL** | Session 138C | Pre-Paid-Beta |
...
> [!IMPORTANT]
> **Conclusion:**
> The repo can run static/build/deterministic QA now, but all DB-mutating, webhook, billing E2E, restore, and load tests remain blocked until a separate staging Supabase project exists and production backups/PITR are enabled.
