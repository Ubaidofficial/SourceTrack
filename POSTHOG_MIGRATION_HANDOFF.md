...
| **D5** | Strip backend `POSTHOG_*` env (all services) + update sub-processor/legal docs → **GDPR claim unlocks** | 🟢 **UNBLOCKED** — all three D2 boot guards are cleared: `api/index.js` REQUIRED_ENV (#304), health-agent (#306), and nightly `:175` (this PR, B3 step 4). No backend code reads `POSTHOG_*` on a runtime path anymore. Ready to strip the env vars from Railway. |
