...
| **D2 · health-agent** | `health-agent.js:137` + `:182` — two direct HogQL fetches (`SELECT 1` liveness; `count($pageview)` last 24h); `:213` requires 4 `POSTHOG_*` vars. `posthog` ∈ `CRITICAL_CHECKS` → an error is a 🔴 critical/Slack alert. | ⬜ **NOT STARTED — DESIGN DECISION, reported not edited.** Once PostHog is decommissioned this check goes 🔴 (false alarm) + `data_flow`/`env_vars` go non-critical error. Equivalent pipes exist but are **tenant-scoped, not global**: liveness ≈ `doctor_token_verify`/`events_health_last`; pageview volume ≈ `events_health_day`/`doctor_pageviews_30d`. Migrate or retire before D5. |
| **D2 · B2** | `nightly-attribution.js:446-507` — suffix/reprocess conversion reads still resolve to the (dead post-D3) HogQL fallback; fail-closed guard blocks the money-rail write today. Migrate these reads onto Tinybird. | ⬜ **NOT STARTED** (separate task; do not start alongside the boot/pkg mechanical work). |
| **D2 · B3** | `nightly-attribution.js` — remove `queryPostHog` and its three fallback call sites (`:511`, `:617`, `:773`); only then remove the `:175` `POSTHOG_PERSONAL_API_KEY`/`POSTHOG_PROJECT_ID` boot guard. | ⬜ **NOT STARTED** — the `:175` guard MUST stay until B3 lands (removing it now turns a config error into a runtime crash on a money-rail path). |
