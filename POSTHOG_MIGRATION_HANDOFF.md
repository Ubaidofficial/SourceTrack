...
| **D2 · B3** | `nightly-attribution.js` — remove `queryPostHog` and its three fallback call sites; only then remove the `:175` `POSTHOG_PERSONAL_API_KEY`/`POSTHOG_PROJECT_ID` boot guard. | 🔶 **IN PROGRESS (2 of 4 steps done).** **Step 1 (#308):** read path retries 429/5xx/network (a null pipe now means "failed after retries", making fail-closed safe). **Step 2 (this PR):** the normal-path **conversions** reads FAIL CLOSED per-site on a null pipe — `processSite` returns `queryFailed:true` (→ `totalHardFailures` → `computeTerminalStatus` 'failed') WITHOUT throwing (per-site isolation), and `fetchBackfillConversions` throws (→ runBackfill's existing exit-1). Neither reads the dead HogQL store on a null anymore; a served-empty `[]` is still a success. **Remaining (step 3/4):** the **touchpoints** read inside `processConversion` and the `queryPostHog` function itself (its two surviving callers: the reads-DISABLED conversions path + touchpoints) — only then can the `:175` guard go. The `:175` guard MUST stay until then. |
