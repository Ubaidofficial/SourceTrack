...
| **D5** | Strip backend `POSTHOG_*` env (all services) + update sub-processor/legal docs → **GDPR claim unlocks** | 🔶 **IN PROGRESS — two halves, tracked separately.** **① ENV half (founder-gated, being inventoried separately):** ⬜ strip `POSTHOG_*` (backend) + `VITE_POSTHOG_*` (frontend, already removed) from Railway. No backend code reads them (all three boot guards cleared: #304/#306/#311). **② CODE/legal-copy half (this PR):** 🔶 Settings.jsx visitor-erasure copy corrected (PostHog deletion mechanism → Tinybird erasure, code-confirmed removed). **BLOCKED on a data-deletion fact:** removing PostHog from `Subprocessors.jsx` and updating the account-deletion retention copy (`Settings.jsx:1393/1394`) require confirming the **PostHog project/data is actually deleted**. Repo evidence says it is **NOT** (`COMMANDCODE_RUNBOOK.md:441/453`: historical events not bulk-deleted; no deletion job/runbook exists) — so those disclosures are **left in place** (removing them while data resides in PostHog would be inaccurate). Founder must confirm project deletion, then the prepared sub-processor removal applies. |
