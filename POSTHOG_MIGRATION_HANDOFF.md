...
| **D2 · B3** | `nightly-attribution.js` — remove `queryPostHog` and its fallback call sites; only then remove the `:175` `POSTHOG_PERSONAL_API_KEY`/`POSTHOG_PROJECT_ID` boot guard. | 🔶 **IN PROGRESS (3 of 4 steps done).** **Step 1 (#308):** read path retries 429/5xx/network (a null pipe now means "failed after retries", making fail-closed safe). **Step 2 (#309):** normal-path **conversions** reads FAIL CLOSED per-site on a null pipe (`processSite` → `queryFailed:true` without throwing; `fetchBackfillConversions` throws → runBackfill exit-1). **Step 3 (this PR):** the **touchpoints** read in `processConversion` FAILS CLOSED on a null pipe — it THROWS so `processSite`'s per-conversion catch SKIPS the conversion (`failed++`, nothing written) instead of writing it with `touchpoint_count:0` (silent mis-attribution). A served-empty `[]` is still written as a genuine no-touchpoint conversion. Also (Half B): `fellBack` is now surfaced at the run level (worker counter + per-site logWarn + summary log + `job_runs.error_message`). **Remaining (step 4):** delete `queryPostHog` (its only remaining callers are the reads-DISABLED conversions + touchpoints legs), the `:175` boot guard, the dead `validateSite` skip guard, and two stale comments — only then is D5 unblocked. |
