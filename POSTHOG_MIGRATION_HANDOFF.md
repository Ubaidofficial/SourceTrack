...
| **D6** | Orphan cleanup (ai-analytics/annotations) folds in | ✅ **DONE.** Deleted the two unmounted orphan routes (`api/routes/annotations.js` — 0-ref; `api/routes/ai-analytics.js` — unmounted, no real PostHog dep) + the dead dashboard page (`dashboard/src/pages/AIAnalytics.jsx`, unrouted) and its two tendrils (`query-error-surfaces.test.js` entry, `Layout.jsx` title). Fixed two stale `'PostHog capture failed'` error labels → provider-neutral (`shopify-webhook.js`, `stripe-webhook.js`). **Untouched (live):** the `ai_analytics` PLAN FEATURE flag (`attribution.js:429`, `plan-features.js:45`) — entitlements unaffected. `site_annotations` table is now fully orphaned (drop is DDL, founder-gated, schedule separately). `admin.js:686/701` AI-Analytics probe left in place (now auto-reports `dormant`). **All PostHog migration items D1–D6 complete.** |
