      ...
      <div className="relative rounded-[28px] bg-[#12100C] p-3 border border-white/10 shadow-[0_24px_80px_rgba(18,16,12,.22)]">
        <div className="overflow-hidden rounded-[20px] bg-[#12100C] border border-[#3D3830]">
          ...
          <div className="h-[44px] flex items-center justify-between px-4 bg-[#1B1811] border-b border-white/10">
            <span className="text-[#F7F4ED] text-xs font-bold">Recent converting journeys</span>
          ...
          <div className="p-4 grid gap-3">
            {JOURNEYS.map((j) => (
              <div key={j.label} className="rounded-2xl bg-[#1B1811] border border-[#3D3830] p-3.5">
                <div className="flex flex-wrap items-center gap-1.5 text-[11px] font-bold text-[#E7E0D2]">
                  <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded bg-[#1B1811] border border-white/5">
                    <SourceIcon source={j.source} className="w-3.5 h-3.5" />{j.label}
                  </span>
                  {j.steps.map((step) => (
                    <span key={step} className="inline-flex items-center gap-1.5">
                      <span className="text-st-lime font-black">→</span>
                      <span className="px-2 py-1 rounded bg-[#1B1811] border border-white/5 font-mono">{step}</span>
                    </span>
                  ))}
                  <span className="text-st-lime font-black">→</span>
                  <span className="px-2 py-1 rounded bg-[#D2EC2A]/20 text-st-lime border border-[#D2EC2A]/20">{j.event}</span>
                </div>
                <div className="mt-2 text-[#A39B8C] text-[10px] font-bold">{j.model}</div>
              </div>
            ))}
          </div>
        </div>
