export default function FeatureCards({ items, compact = false }) {
  return (
    <div className={`grid gap-[18px] ${items.length === 4 ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>
      {items.map((f, i) => (
        <article key={i} className={`relative overflow-hidden p-7 rounded-[28px] bg-white border border-[rgba(31,35,35,.10)] shadow-[0_12px_38px_rgba(31,35,35,.055)] ${compact ? 'min-h-[220px]' : 'min-h-[300px]'}`}>
