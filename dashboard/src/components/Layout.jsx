    ...
    <div className="flex h-screen bg-[#F9FAFB]">
      ...
      {location.pathname !== '/onboarding' && (
        <>
          ...
          {/* Slide-in panel */}
          <div className={`fixed bottom-0 right-0 z-40 flex flex-col bg-white border-l border-t border-gray-200 shadow-2xl rounded-tl-2xl transition-all duration-300 ease-in-out ${
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 flex-shrink-0">
              ...
              <button onClick={() => setAiChatOpen(false)} className="p-1 text-st-gray hover:text-st-black rounded">
                <X className="w-4 h-4" />
              </button>
            </div>
            <AIChatPanel />
          ...
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm leading-relaxed ${
              m.role === 'user'
                ...
                : 'bg-gray-100 text-st-black rounded-bl-sm'
            }`}>
              {m.content}
            </div>
          </div>
      ...
      <div className="px-3 py-3 border-t border-gray-100 flex-shrink-0">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-st-black/20 disabled:opacity-50"
          />
          <button
            onClick={handleSend}
        ...
        {!siteKey && !loading && (
          <p className="text-[10px] text-st-gray mt-1">Complete setup to enable AI chat.</p>
        )}
      </div>
    </>
  )
