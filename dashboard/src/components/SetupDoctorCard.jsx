  ...
  return (
    <div className="bg-white dark:bg-[#1A1D1D] border border-gray-200 dark:border-[#2A2E2E] rounded-xl p-5 shadow-sm space-y-4">
      ...
      {recommended_next_action && recommended_next_action.action_key !== 'none' && (
        <div className="p-3 bg-amber-50/50 dark:bg-amber-955/10 border border-amber-200/40 dark:border-amber-900/30 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
      ...
      {mode === 'snippet' && (
        <div className="pt-3 border-t border-gray-150 dark:border-[#2A2E2E] space-y-2.5">
          ...
          {!diagnostics?.domain_match?.registered_domain ? (
            <p className="text-xs text-amber-600 dark:text-amber-450 flex items-center gap-1">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                {testPageOpened && !diagnostics?.verification_token?.token_matched && (
                  <button
                    ...
                    onClick={() => refetch()}
                    disabled={isFetching}
                    className="px-3 py-2 bg-gray-150 dark:bg-[#252929] hover:bg-gray-200 dark:hover:bg-[#333838] text-gray-700 dark:text-gray-250 text-xs font-bold rounded-lg transition-colors border border-gray-200 dark:border-white/5 flex items-center justify-center gap-1"
                  >
                    <RefreshCw className={`w-3 h-3 ${isFetching ? 'animate-spin' : ''}`} />
                    Check now
                  </button>
              ...
              <div className="text-[11px] font-semibold">
                {diagnostics?.verification_token?.token_matched ? (
                  ...
                  <span className="text-amber-600 dark:text-amber-450 flex items-center gap-1">
      ...
      {mode === 'snippet' && (
        <div className="pt-3 border-t border-gray-150 dark:border-[#2A2E2E]">
          <details className="group" onToggle={(e) => {
            ...
            <div className="mt-3 p-3 bg-gray-50 dark:bg-[#1E2121]/50 border border-gray-200 dark:border-transparent rounded-lg text-xs space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-gray-650 dark:text-gray-400 font-medium">Connection to SourceTrack API:</span>
