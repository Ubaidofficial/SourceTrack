export default function ReportBuilderMock() {
  return (
    <div className="relative min-h-[620px]">
      ...
      <div className="relative rounded-[34px] p-[18px] bg-[#F1EDE3] border border-[#E7E0D2] shadow-[0_24px_80px_rgba(18,16,12,.12)]">
      ...
      <div className="absolute left-[-24px] bottom-[34px] w-[292px] rounded-[24px] p-[18px] bg-white/90 border border-[rgba(18,16,12,.10)] shadow-[0_24px_80px_rgba(18,16,12,.12)] backdrop-blur-lg" style={{ transform: 'rotate(2deg)' }}>
        ...
        <p className="mt-2 text-[#6E675C] text-[13px] font-semibold">Build, save, and pin only the reports your team actually needs to make decisions.</p>
      </div>
    </div>
  )
}
...
function Widget({ bars, list, title }) {
  return (
    <div className="col-span-3 min-h-[150px] p-[18px] rounded-[22px] bg-white border border-[#E7E0D2]">
      ...
      {list && (
        <div className="grid gap-[10px]">
          {[92, 78, 84, 62].slice(0, list === 1 ? 4 : 3).map((w, i) => (
            <span key={i} className="block h-3 rounded-full bg-[#F7F4ED]" style={{ width: `${w}%` }} />
          ))}
        </div>
      )}
    </div>
