    ...
    <div className="st-container space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Privacy-friendly, cookieless web analytics</p>
        </div>
        <div className="flex items-center gap-1.5">
          {TIME_RANGES.map(tr => (
            <button key={tr.days} onClick={() => setDays(tr.days)}
              className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${
                days === tr.days ? 'bg-gray-900 text-white' : 'text-gray-500 hover:bg-gray-100'
              }`}>
              {tr.label}
            </button>
          ))}
      ...
      {isLoading ? (
        ...
        <div className="max-w-lg mx-auto py-16 text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Add the analytics tracker to your site to start collecting pageview data.</p>
          </div>
          ...
          <p className="text-xs text-gray-400">Cookieless · No consent banner required · GDPR compliant</p>
        </div>
      ) : (
        <>
              ...
              {topSources.length === 0
                ...
                : topSources.map((r, i) => (
                  <div key={i} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                    ...
                    <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${r.source.startsWith('AI:') ? 'bg-lime-500' : 'bg-gray-900'}`}
            ...
            <DashboardCard title="AI Traffic" subtitle="Visits from AI platforms"
              ...
              {aiSources.length === 0 ? (
                <div className="py-6 text-center">
                  ...
                  <p className="text-xs text-gray-400">No AI traffic yet</p>
                </div>
              ) : aiSources.map((r, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <span className="text-sm text-gray-700 font-medium">{r.source}</span>
                  <span className="text-sm font-semibold text-gray-900">{r.visits.toLocaleString()}</span>
                </div>
              ))}
            </DashboardCard>
          ...
          <DashboardCard title="Analytics Snippet" subtitle="Cookieless · No consent banner · GDPR compliant">
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-center gap-3">
              ...
              <button onClick={() => navigator.clipboard.writeText(snippetUrl)}
                className="text-xs text-gray-500 hover:text-gray-900 px-2 py-1 border border-gray-200 rounded flex-shrink-0">
                Copy
              </button>
            </div>
          </DashboardCard>
