      ...
      <div className="space-y-6">
        ...
        <section className="space-y-2">
          ...
          <ParamTable params={[
            ...
            { name: 'event_id', type: 'string', required: false, desc: 'Shared deduplication ID for Meta and TikTok reporting. Must be byte-identical to the eventID on your browser pixel call, or the ad platform counts the conversion twice. Must also be unique per EVENT, not per order: when you supply this value it replaces the ID we would otherwise derive, and the derived one includes the conversion type while yours does not — so sending two events of different types for the same order under one event_id makes them collide, and one is silently dropped. If a single order raises more than one conversion, give each its own event_id. Truncated to the first 200 characters, and any character outside A-Z a-z 0-9 : . _ - is removed.' },
