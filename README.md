...
| Email reports | `0 8 * * 1` | `node api/jobs/email-reports.js` | ⚠️ **Had never sent an email — fixed 2026-07-27. It was NOT crashing.** Every run completed and wrote `job_runs` `status='success'`, `"Sent 0, skipped 4, errors 0"` — a boot crash cannot write a success row with counts. The real cause: owner-email resolution queried `public.users`, **a table that does not exist** (emails live in `auth.users`), the error was discarded, and every site fell through to "no owner email → skip". A second bug filtered `company_members.company_id` by a **site** id. Both fixed; owner email now resolves via `auth.admin.getUserById`, and a failed read is counted as an error instead of masquerading as "nobody to email". **NOT YET LIVE:** the `sourcetrack-email` service is disconnected from GitHub, so prod keeps running the last-deployed build until someone reconnects it — merging the fix does not start the emails. **Before reconnecting, settle the schedule:** `job_runs` shows this job firing several times a day against a documented `0 8 * * 1`, which was harmless only while it sent nothing. |
