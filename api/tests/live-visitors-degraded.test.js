...
//
// These tests pin the distinction: [] => degraded false (truthful empty), null => degraded
// true (unknown), throw => 500 (a real error is not a 200). If someone reintroduces
// `rows ?? []`, the null case flips degraded to false and this fails.
...
test('a thrown read is a 500, never a success with an empty room', async () => {
  __setLiveReadDeps({ queryTinybird: async () => { throw new Error('pipe exploded') } })
  try {
    const res = await invoke()
    assert.equal(res.statusCode, 500, 'a real error must not be answered with 200')
    assert.equal(res.body.success, false)
    assert.equal(res.body.data, null, 'no empty visitor list that the panel could render')
    assert.equal(res.body.error, 'pipe exploded')
  } finally { __resetLiveReadDeps() }
})
