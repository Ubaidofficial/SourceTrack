// flexible_report CAMPAIGN dim-swap — getFlexibleReport dispatch + the STRICT gate.
...
// page (campaigns.js: model=last_touch, no window -> dead HogQL -> zeros). Everything else
// (non-direct models, a window, non-campaign dim, filters, dim2, non-UTC, non-base metric) must
// fall through to HogQL and MUST NOT hit the campaign pipe.
...
const HOG_ROWS = [['summer_sale', 5], ['brand', 3]]
...
test('DISPATCH: last_touch + campaign — pipe named rows == HogQL positional (byte-identical)', async () => {
  const pipeRes = await run({
    queryTinybird: async (pipe) => pipe === PIPE ? PIPE_ROWS : null,
    queryHog: async (_sql, name) => { if (name === 'flexible_report') throw new Error('campaign base case must come from the pipe, not HogQL'); return [] }
  })
  ...
  assert.strictEqual(pipeRes.find((r) => r.dim_value === 'summer_sale').conversions, 5, 'metric carried through')
})
...
test('DISPATCH: first_touch + campaign — pipe serves it too (model param switches the column)', async () => {
  ...
  const res = await run({
    ...
    queryHog: async (_sql, name) => { if (name === 'flexible_report') throw new Error('first_touch campaign base case must come from the pipe'); return [] }
  }, { model: 'first_touch' })
...
test('DISPATCH: revenue metric threaded (MONEY-RAIL) — pipe served, no HogQL', async () => {
  ...
  const res = await run({
    ...
    queryHog: async (_sql, name) => { if (name === 'flexible_report') throw new Error('revenue base case must come from the pipe'); return [] }
  }, { metric: 'revenue' })
  const hit = seen.find((s) => s.pipe === PIPE)
...
test('FALLBACK: pipe null -> HogQL flexible_report is called and its rows returned', async () => {
  ...
  assert.strictEqual(res.find((r) => r.dim_value === 'summer_sale').conversions, 5, 'HogQL rows surface unchanged')
})
...
// ── HELD: non-direct models must NOT dispatch (parity held; utm_campaign _nd COALESCE differs) ──
for (const model of ['first_touch_non_direct', 'last_touch_non_direct']) {
  test(`GATE: ${model} + campaign does NOT dispatch (non-direct is HELD)`, async () => {
    const pipes = []
    await run({
...
test('WINDOW: campaign + attribution window does NOT dispatch (window re-attributes campaign; pipe would diverge)', async () => {
  const pipes = []
  await run({
    queryTinybird: async (p) => { pipes.push(p); return null },
    queryHog: async (_sql, name) => name === 'flexible_report' ? HOG_ROWS : []
...
// ── THE GATE: non-base shapes MUST fall through and NEVER hit the campaign pipe ──
for (const shape of [
  ...
  test(`GATE: ${shape.name} does NOT dispatch the campaign pipe`, async () => {
    const pipes = []
    await run({
      queryTinybird: async (p) => { pipes.push(p); return null },
      queryHog: async (_sql, name) => name === 'flexible_report' ? HOG_ROWS : []
